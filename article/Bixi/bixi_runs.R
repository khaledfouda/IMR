...
#---
convergence <- imr_convergence(maxit = 1000, thresh=1e-5, trace=FALSE, ls_initial = TRUE)
grid <- imr_tune_grid(nuclear = c(0,30,80, 5), rank = c(2,30,1, 5)); grid
  ...
  for (prefix in 1:50) {
    for (train_size in train_seq) {
      for (i in 1:nrow(model_combn)) {
        ...
        grid <- IMR::imr_set_grid_limits(model_data, grid, default_rank=10, convergence=convergence,
        ...
        res <- data.frame(
          ...
          lambda_nuclear = fitimr$fit$meta$lambdas["M"],
      ...
      total_results %>%
        ...
        mutate(time = round(time, 2)) %>% print()
    }
...
total_results <- readRDS("./article/Bixi/data/final_results/IMR_results_final_25pctf.rds")
total_results %>%
  filter(metric == "RRMSE") %>%
  group_by(model, train_size, metric) %>%
  summarise_all(mean) %>%
...
total_results %>%
  ...
  slice_head(n=1) %>%
  ungroup() %>%
  select(model, lambda_nuclear, rank_estim) %>%
  transmute(similarity = model == "similarity",
            lambda=lambda_nuclear,
  ...
  for (train_size in train_seq) {
    for (i in 1:nrow(model_combn)) {
      ...
      res <- data.frame(
        ...
        prefix = prefix,
        test_pct = test_pct,
        lambda_nuclear = model_combn$lambda[i],
