...
#'   of the row similarity matrix, typically generated by \code{\link{imr_similarity}}.
#'   Defaults to `NULL`.
#' @param similarity_cols A list containing the eigenvalues (`d`) and eigenvectors (`U`)
#'   of the column similarity matrix, typically generated by \code{\link{imr_similarity}}.
#'   Defaults to `NULL`.
...
#' @param data An object of class `"imr_data"` containing the matrices and model
#'   structure used during fitting. This is required to access the QR decompositions
#'   for back-transforming coefficients.
...
#' based on the model structure: the low-rank matrix ($M = U D V^T$), row covariate
#' effects ($X \beta$), column covariate effects ($\Gamma Z$), and any intercepts.
...
#'
#' @param fit An object of class `"imr_fit"`, typically generated by \code{\link{imr_fit}}.
#' @param data An object of class `"imr_data"` containing the covariates used during
#'   fitting.
#' @param target An `Incomplete` (sparse) matrix. The function uses the structural
#'   row and column indices (`@i` and `@p`) of this matrix to determine exactly
#'   where to compute predictions. The actual numerical values inside this matrix
#'   do not matter, as they will be overwritten.
...
#' every cell of an $n \times m$ matrix, `reconstruct_partial` relies on optimized C++
#' routines to calculate only the targeted entries.
#'
#' @return The original `target` sparse matrix, but with its values (`@x` slot)
