...
for(miss in c(.6,.7,.8,.9,.95, .55,.65, .75, .85, .99)){
  ...
  z_vars <- c("z_walkscore", "z_len_minor_road", "z_num_restaurants",
...
dat <- IMR::prepare_data(odat$Y.inc, odat$X, odat$Z, 0.2, rand)
...
saveRDS(fit.imr, paste0("./article_results/bixi/data/imr_",round(100*miss),
                        "_intercept_fit.rds"))
}
}
}
...
for(miss in c(.6,.7,.8,.9,.95, .55,.65, .75, .85, .99)){
  ...
  fit.imr <- readRDS(paste0("./article_results/bixi/data/imr_",round(100*miss),
                            "_intercept_fit.rds"))
  out <- IMR:::reconstruct(fit.imr$fit, dat)
  prepare_output_bixi(
    fit.imr$time*60,
  ...
  #--- Simpute
  fit.si <- readRDS(paste0("./article_results/bixi/data/SI_",round(100*miss),
                            "_fit.rds"))
  ...
  res.imr[-10] %>%
    ...
    cbind(miss=miss,model=c("IMR", "IMR-Intercept", "SoftImpute", "BKTR"))
                     )
...
cols <- c("IMR"="#56B4E9", "IMR-Intercept" = "#0072B2",
...
# shp  <- c("IMR"=16, "SoftImpute"=17, "BKTR"=15)
  ...
  results.df %>%
    mutate(time = (time/60)) %>%
  mutate(
     model = factor(model, levels = c("IMR", "IMR-Intercept", "SoftImpute", "BKTR"))
  ) %>%
  ...
  ggsave("./article_results/bixi/data/plot_bixi.png",
         g1, width = 320/25.4, height = 150/25.4, dpi = 600)
#=============================================================
...
hist_with_density <- function(x, xlab, binwidth, color_bars = "#B9C6D6", color_lines = "#2F4858") {
  ...
  y_max <- hist(x, breaks = seq(floor(min(x)/binwidth) * binwidth,
                                ceiling(max(x)/binwidth) * binwidth, by = binwidth),
                plot = FALSE)$counts |> max(0)
  ...
  ggplot(data.frame(x = x), aes(x)) +
    ...
    annotate("text", x = qs[2], y = y_max, label = paste0("Med = ", round(qs[2], 1)),
             vjust = -0.4, size = 3.2) +
    labs(x = xlab, y = "Days") +
    theme_paper()
}
...
timeseries_loess <- function(dates, y, ylab, color_line = "#2F4858", color_smooth = "#BC412B") {
  ...
  x_min <- min(dates); x_max <- max(dates)
...
# fig_bixi_daycovs <- (p_hum_hist + p_hum_ts) / (p_tmp_hist + p_tmp_ts) +
  ...
  ggsave("./article_results/bixi/data/plot_bixi_cov_x.png",
         fig_bixi_daycovs, width = 320/25.4, height = 150/25.4, dpi = 600)
...
p_station_hist <- ggplot(z_long, aes(value)) +
  ...
  labs(
    ...
    subtitle = paste0("Per-station histograms; ",
...
ggsave("./article_results/bixi/data/plot_bixi_cov_z.png",
       p_station_hist, width = 320/25.4, height = 150/25.4, dpi = 600)
#===========================================================
out <- IMR:::reconstruct(fit.imr$fit, dat)
